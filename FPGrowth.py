...
def createTree(dataSet, min_sup=1):  # create FP-tree from dataset but don't mine
    headerTable = {}
...
def mineTree(inTree, headerTable, min_sup, preFix, freqItemList):
    ...
    for basePat in bigL:  # start from bottom of header table
        ...
        myCondTree, myHead = createTree(condPattBases, min_sup)
        # print 'head from conditional tree: ', myHead
        if myHead is not None:  # 3. mine cond. FP-tree
            # print 'conditional tree for: ',newFreqSet
            # myCondTree.disp(1)
            mineTree(myCondTree, myHead, min_sup, newFreqSet, freqItemList)  # 递归
...
def FPGrowth(dataset, min_sup):
    ...
    if myFPtree is not None:
        # myFPtree.disp()
        mineTree(myFPtree, myHeaderTab, min_sup, set(), myFreqList)
    return myFreqList
